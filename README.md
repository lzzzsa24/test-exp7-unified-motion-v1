...
5 ms 周期直接输出左右侧 PWM。中间微调、外侧急转、丢线持续搜索，直到人工停车。
...
本次取消自动停车的修订尚未烧录，串口版本标识为 `SL2`。
用户最新反馈为原程序丢线和有线时都会反复停车，板上镜像哈希尚未核对。
...
取消自动停车前的回退标签为 `rollback/2026-09-05-before-continuous-simple-line`。
本分支未推送远端。
